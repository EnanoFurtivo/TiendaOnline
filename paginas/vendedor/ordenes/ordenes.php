...
<script src="paginas/vendedor/ordenes/ordenes.js"></script>
  ...
  <table class="table table-hover table-bordered d-flex flex-column flex-grow-1 overflow-hidden align-middle scrollable-table" id='ordenes-table'>
    ...
    <tbody class="d-flex flex-column flex-grow-1" id="ordenes-table-content">
      <?php
        ...
        $ordenes = $vendedor->getOrdenes();

        foreach ($ordenes as $key => $orden) {
          
          $id = $orden->id;
          $nro_productos = $orden->cant_items;
          $comprador = $orden->comprador->username;
          $estado = $orden->estado;
          $fecha = $orden->fecha;
          ...
          echo '<td class="td-centered">'.$comprador.'</td>';
          echo '<td class="td-centered">'.$estado.'</td>';
          echo '<td class="td-centered">'.$fecha.'</td>';

          //ACCIONES//
          echo '<td class="td-centered">';
          echo '<button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#modal-ordenes" onclick="vendedor_ordenes.abrir_modal('.$id.');">Ver Detalle</button>';
          echo '</td>';
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <form class="form-window form-frame" onsubmit="return false;">
            <label for="basic-url" class="form-label">Datos del comprador</label>
            <div class="input-group mb-3">
              <span class="input-group-text">Nombre
              </span>
              <input type="text" class="form-control" id="modal-ordenes-NOMBRE" disabled>
            </div>
            <div class="input-group mb-3">
              <span class="input-group-text">Email
              </span>
              <input type="text" class="form-control" id="modal-ordenes-EMAIL" disabled>
            </div>
            <div class="input-group mb-3">
              <span class="input-group-text">Telefono
              </span>
              <input type="text" class="form-control" id="modal-ordenes-TELEFONO" disabled>
            </div>
          </form>
        ...
        <div class="modal-footer">
          ...
          <button type="button" class="btn btn-success" id="productos-modal-footer-submit" onclick="vendedor_ordenes.finalizar_orden();">Finalizar Orden</button>
        </div>
